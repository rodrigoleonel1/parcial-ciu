...
export default function Catalogo() {
  ...
  const productosFiltrados = productos.filter((producto) => {
    ...
    const coincideCategoria =
      ...
      filtros.categoriasSeleccionadas.includes(producto.categoria);
  ...
  return (
    <div className="container mx-auto py-8">
      <h1 className="text-center font-display font-bold text-5xl sm:text-6xl lg:text-7xl mb-5">
        Nuestros <span className="text-cyan-400">Productos</span>
      </h1>

      <button
        onClick={() => setMostrarFiltros(true)}
        className="md:hidden bg-gray-700 text-white px-4 py-2 rounded mb-4 ml-4"
      >
        Filtrar
      </button>
      ...
      <div className="flex gap-8 p-5 m-auto justify-center">
        ...
        <div className="grid flex-1 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 min-h-[70vh]">
          {productosOrdenados.length === 0 ? (
            <p className="text-center text-lg text-gray-400 drop-shadow-[0_0_15px_rgba(59,130,246,0.8)] col-span-full">
